...
@section('title', 'Edit Category - Money Manager')
                        ...
                        <div class="card-body">
                            <form action="{{ route('category.update', $category['data']['id']) }}" method="POST">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <input class="form-control" type="text" placeholder="Input nama kategori" name="name_category" value="{{ $category['data']['name_category'] }}" required>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <select class="form-control select2 select" name="id_transaction_type">
                                            ...
                                            @foreach ($transactionTypes['data'] as $transactionType)
                                                <option value="{{ $transactionType['id'] }}" {{ old('id_transaction_type', $category['data']['id_transaction_type']) == $transactionType['id'] ? 'selected' : '' }}>{{ $transactionType['name_transaction_type'] }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
